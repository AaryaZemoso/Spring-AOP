package aopdemo;

import aopdemo.service.TrafficFortuneService;
import org.springframework.context.annotation.AnnotationConfigApplicationContext;

import java.util.logging.Logger;

public class MainDemoAroundHandleExceptionsApp {

    private static Logger logger = Logger.getLogger(MainDemoAroundHandleExceptionsApp.class.getName());

    public static void main(String[] args) {

        // Read config
        AnnotationConfigApplicationContext applicationContext =
                new AnnotationConfigApplicationContext(DemoConfig.class);

        // Get the bean from spring
        TrafficFortuneService tf = applicationContext.getBean("trafficFortuneService", TrafficFortuneService.class);

        try {
            boolean tripWire = true;
            logger.info(tf.getFortune(tripWire));
        } catch (Exception e){
            logger.severe("Exception occurred : " + e);
        }
        // Close context
        applicationContext.close();
    }
}
